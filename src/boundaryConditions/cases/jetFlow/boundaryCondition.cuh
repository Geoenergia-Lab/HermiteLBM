...
moments[m_i<1>()] = (is_outlet * shared_buffer[tid * (NUMBER_MOMENTS<false>() + 1) + m_i<1>()]) + (is_jet * (device::U_Back[0] + ux));
moments[m_i<2>()] = (is_outlet * shared_buffer[tid * (NUMBER_MOMENTS<false>() + 1) + m_i<2>()]) + (is_jet * (device::U_Back[1] + uy));

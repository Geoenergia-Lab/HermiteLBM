...
case normalVector::WEST_NORTH():
{
    ...
    {
        moments[m_i<6>()] = (static_cast<scalar_t>(6) * mxz_I + moments[m_i<3>()]) / (static_cast<scalar_t>(3)); // mxz
    }
    else
    {
        moments[m_i<6>()] = (static_cast<scalar_t>(45) * mxz_I + static_cast<scalar_t>(9) * myz_I + 5 * moments[m_i<3>()]) / (static_cast<scalar_t>(18)); // mxz
    }
    moments[m_i<7>()] = moments[m_i<2>()] * moments[m_i<2>()]; // myy
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<8>()] = (static_cast<scalar_t>(6) * myz_I - moments[m_i<3>()]) / (static_cast<scalar_t>(3)); // myz
    }
    else
    {
        moments[m_i<8>()] = (static_cast<scalar_t>(9) * mxz_I + static_cast<scalar_t>(45) * myz_I - 5 * moments[m_i<3>()]) / (static_cast<scalar_t>(18)); // myz
    }
    moments[m_i<9>()] = moments[m_i<3>()] * moments[m_i<3>()]; // mzz
...
case normalVector::WEST_FRONT():
{
    ...
    {
        moments[m_i<5>()] = (static_cast<scalar_t>(45) * mxy_I + static_cast<scalar_t>(9) * myz_I + 5 * moments[m_i<2>()]) / (static_cast<scalar_t>(18)); // mxy
    }
    moments[m_i<6>()] = moments[m_i<1>()] * moments[m_i<3>()]; // mxz
    moments[m_i<7>()] = moments[m_i<2>()] * moments[m_i<2>()]; // myy
    if constexpr (VelocitySet::Q() == 19)
    ...
    {
        moments[m_i<8>()] = (static_cast<scalar_t>(9) * mxy_I + static_cast<scalar_t>(45) * myz_I - 5 * moments[m_i<2>()]) / (static_cast<scalar_t>(18)); // myz
    }
    moments[m_i<9>()] = moments[m_i<3>()] * moments[m_i<3>()]; // mzz
...
case normalVector::NORTH_FRONT():
{
    ...
    {
        moments[m_i<5>()] = (static_cast<scalar_t>(45) * mxy_I - static_cast<scalar_t>(9) * mxz_I - 5 * moments[m_i<1>()]) / (static_cast<scalar_t>(18)); // mxy
    }
    if constexpr (VelocitySet::Q() == 19)
    {
    ...
    {
        moments[m_i<6>()] = (-static_cast<scalar_t>(9) * mxy_I + static_cast<scalar_t>(45) * mxz_I - 5 * moments[m_i<1>()]) / (static_cast<scalar_t>(18)); // mxz
    }
    moments[m_i<7>()] = moments[m_i<2>()] * moments[m_i<2>()]; // myy
    moments[m_i<8>()] = moments[m_i<2>()] * moments[m_i<3>()]; // myz
    moments[m_i<9>()] = moments[m_i<3>()] * moments[m_i<3>()]; // mzz
...
case normalVector::NORTH():
{
    ...
    const scalar_t mzz_I = velocitySet::calculate_moment<VelocitySet, Z, Z>(pop, boundaryNormal);
    const scalar_t mxy_I = velocitySet::calculate_moment<VelocitySet, X, Y>(pop, boundaryNormal);
    const scalar_t mxz_I = velocitySet::calculate_moment<VelocitySet, X, Z>(pop, boundaryNormal);
    const scalar_t myz_I = velocitySet::calculate_moment<VelocitySet, Y, Z>(pop, boundaryNormal);
    ...
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<4>()] = (4 * mxx_I - 4 * mzz_I + 3 * moments[m_i<1>()] * moments[m_i<1>()] + 3 * moments[m_i<3>()] * moments[m_i<3>()]) / (static_cast<scalar_t>(6)); // mxx
    }
    else
    {
        moments[m_i<4>()] = (6 * mxx_I - 6 * mzz_I + 5 * moments[m_i<1>()] * moments[m_i<1>()] + 5 * moments[m_i<3>()] * moments[m_i<3>()]) / (static_cast<scalar_t>(10)); // mxx
    }
    moments[m_i<5>()] = (static_cast<scalar_t>(6) * mxy_I - moments[m_i<1>()]) / (static_cast<scalar_t>(3)); // mxy
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<6>()] = mxz_I; // mxz
    }
    else
    {
        moments[m_i<6>()] = (static_cast<scalar_t>(6) * mxz_I) / (static_cast<scalar_t>(5)); // mxz
    }
    moments[m_i<7>()] = moments[m_i<2>()] * moments[m_i<2>()];                                               // myy
    moments[m_i<8>()] = (static_cast<scalar_t>(6) * myz_I - moments[m_i<3>()]) / (static_cast<scalar_t>(3)); // myz
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<9>()] = (4 * mzz_I - 4 * mxx_I + 3 * moments[m_i<1>()] * moments[m_i<1>()] + 3 * moments[m_i<3>()] * moments[m_i<3>()]) / (static_cast<scalar_t>(6)); // mzz
    }
    else
    {
        moments[m_i<9>()] = (6 * mzz_I - 6 * mxx_I + 5 * moments[m_i<1>()] * moments[m_i<1>()] + 5 * moments[m_i<3>()] * moments[m_i<3>()]) / (static_cast<scalar_t>(10)); // mzz
    }
...
case normalVector::FRONT():
{
    ...
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<4>()] = (4 * mxx_I - 4 * myy_I + 3 * moments[m_i<1>()] * moments[m_i<1>()] + 3 * moments[m_i<2>()] * moments[m_i<2>()]) / (static_cast<scalar_t>(6)); // mxx
    }
    else
    {
        moments[m_i<4>()] = (6 * mxx_I - 6 * myy_I + 5 * moments[m_i<1>()] * moments[m_i<1>()] + 5 * moments[m_i<2>()] * moments[m_i<2>()]) / (static_cast<scalar_t>(10)); // mxx
    }
    if constexpr (VelocitySet::Q() == 19)
    {
    ...
    if constexpr (VelocitySet::Q() == 19)
    {
        moments[m_i<7>()] = (4 * myy_I - 4 * mxx_I + 3 * moments[m_i<1>()] * moments[m_i<1>()] + 3 * moments[m_i<2>()] * moments[m_i<2>()]) / (static_cast<scalar_t>(6)); // myy
    }
    else
    {
        moments[m_i<7>()] = (6 * myy_I - 6 * mxx_I + 5 * moments[m_i<1>()] * moments[m_i<1>()] + 5 * moments[m_i<2>()] * moments[m_i<2>()]) / (static_cast<scalar_t>(10)); // myy
    }
    moments[m_i<8>()] = (static_cast<scalar_t>(6) * myz_I - moments[m_i<2>()]) / (static_cast<scalar_t>(3)); // myz
    moments[m_i<9>()] = moments[m_i<3>()] * moments[m_i<3>()];                                               // mzz

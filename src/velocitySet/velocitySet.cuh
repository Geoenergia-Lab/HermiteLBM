...
namespace LBM
{
    ...
    public:
         ...
         **/
        template <class VelocitySet>
        __device__ __host__ static inline void calculate_moments(const thread::array<scalar_t, VelocitySet::Q()> &pop, thread::array<scalar_t, NUMBER_MOMENTS<false>()> &mom) noexcept
        {
            // Density
        ...
        __device__ __host__ static inline void calculate_moments(const thread::array<scalar_t, VelocitySet::Q()> &pop, thread::array<scalar_t, NUMBER_MOMENTS<false>()> &mom, const BoundaryNormal &boundaryNormal) noexcept
        {
            // Density
        ...
        template <class VelocitySet>
        __device__ __host__ static inline void calculate_moments(const thread::array<scalar_t, VelocitySet::Q()> &pop, thread::array<scalar_t, NUMBER_MOMENTS<true>()> &mom) noexcept
        {
            // Density
            mom[m_i<0>()] = calculate_moment<VelocitySet, axis::NO_DIRECTION, axis::NO_DIRECTION>(pop);
            const scalar_t inv_rho = static_cast<scalar_t>(1) / mom[m_i<0>()];
        ...
        template <class VelocitySet, class BoundaryNormal>
        __device__ __host__ static inline void calculate_moments(const thread::array<scalar_t, VelocitySet::Q()> &pop, thread::array<scalar_t, NUMBER_MOMENTS<true>()> &mom, const BoundaryNormal &boundaryNormal) noexcept
        {
            // Density
            mom[m_i<0>()] = calculate_moment<VelocitySet, axis::NO_DIRECTION, axis::NO_DIRECTION>(pop, boundaryNormal);
            const scalar_t inv_rho = static_cast<scalar_t>(1) / mom[m_i<0>()];

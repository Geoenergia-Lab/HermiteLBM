...
namespace LBM
{
    namespace phaseFieldSponge
    {
        ...
        {
            return static_cast<scalar_t>(static_cast<scalar_t>(sponge_cells()) / static_cast<scalar_t>(device::nz - static_cast<device::label_t>(1)));
        }

        __device__ [[nodiscard]] inline scalar_t z_start() noexcept
        {
            return static_cast<scalar_t>(static_cast<scalar_t>(device::nz - static_cast<device::label_t>(1) - sponge_cells()) / static_cast<scalar_t>(device::nz - static_cast<device::label_t>(1)));
        }
        ...
        __device__ [[nodiscard]] inline scalar_t ramp_zmax(const device::label_t zGlobal) noexcept
        {
            ...
            scalar_t s = (zn - z_start()) * inv_sponge();
        ...
        __device__ [[nodiscard]] inline scalar_t tau_zmax(const scalar_t phi) noexcept
        {
        ...
        __device__ [[nodiscard]] inline scalar_t tau(const scalar_t phi, const device::label_t zGlobal) noexcept
        {
            ...
            {
                const scalar_t r = ramp_zmax(zGlobal);
                return tauPhi + r * (tau_zmax(phi) - tauPhi);
            }

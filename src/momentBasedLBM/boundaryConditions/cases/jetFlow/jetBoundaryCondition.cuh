...
case normalVectorBase::BACK():
{
    ...
    // constexpr const scalar_t is_jet = static_cast<scalar_t>(1);

    constexpr const scalar_t U_x = static_cast<scalar_t>(0);
    constexpr const scalar_t U_y = static_cast<scalar_t>(0);
    const scalar_t U_z = is_jet * device::U_Back[2];
    genericDirichlet<VelocitySet>::apply<normalVectorBase::BACK()>(moments, incomings, U_x, U_y, U_z);
